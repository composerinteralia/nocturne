...
class ClientTest < NocturneTest
  ...
  def test_nocturne_connect_tcp_string_host
  ...
  def test_nocturne_multiple_results_disabled
    ...
    e = assert_raises(Nocturne::Error) do
  ...
  #   create_test_table(client)
  #
  #   client.query("INSERT INTO nocturne_test (date_time_test) VALUES ('4321-01-01 00:00:00')")
  #   client.query("INSERT INTO nocturne_test (date_time_test) VALUES ('1234-00-00 00:00:00')")
  #   client.query("INSERT INTO nocturne_test (date_time_test) VALUES ('2345-12-31 00:00:00')")
  #
  #   err = assert_raises Nocturne::Error do
  #     client.query("SELECT date_time_test FROM nocturne_test")
  #   end
  ...
  #   create_test_table(client)
  #
  #   client.query("INSERT INTO nocturne_test (varchar_test, int_test) VALUES ('a', 2)")
  #   client.query("INSERT INTO nocturne_test (varchar_test, int_test) VALUES ('b', 2)")
  ...
  #
  #   assert_raises_connection_error do
  #     client.query("SELECT varchar_test FROM nocturne_test WHERE int_test = 2").to_a
  #   end
  # end
  #
  ...
  def test_query_error
    ...
    err = assert_raises Nocturne::QueryError do
  ...
  #   fake_server.close
  #
  #   assert_raises Nocturne::ConnectionError do
  #     new_tcp_client(host: "127.0.0.1", port: fake_port)
  #   end
  # end

  # def test_connection_invalid_dns
  #   ex = assert_raises Nocturne::ConnectionError do
  #     new_tcp_client(host: "mysql.invalid", port: 3306)
  #   end
  #   assert_equal "nocturne_connect - unable to connect to mysql.invalid:3306: TRILOGY_DNS_ERROR", ex.message
  # end

  def test_nocturne_int_sum_query
    client = new_tcp_client
    create_test_table(client)
    ...
    result = client.query("SELECT SUM(int_test) FROM nocturne_test")
    sum = result.rows[0][0]
  ...
  def test_nocturne_decimal_sum_query
    client = new_tcp_client
    create_test_table(client)

    client.query("INSERT INTO nocturne_test (decimal_test) VALUES ('4')")
    client.query("INSERT INTO nocturne_test (decimal_test) VALUES ('3')")
    client.query("INSERT INTO nocturne_test (decimal_test) VALUES ('1')")

    result = client.query("SELECT SUM(decimal_test) FROM nocturne_test")
    sum = result.rows[0][0]
  ...
  #   assert_operator Nocturne::ConnectionError, :===, klass.new
  # end
end
