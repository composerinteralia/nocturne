...
class SslTest < NocturneTest
  ...
  def tcp_client_defaults
    super.merge(ssl_mode: Nocturne::SSL_REQUIRED_NOVERIFY)
  end
  ...
  def test_nocturne_connect_ssl_config_tls13
    return skip unless tls_1_3_support?

    client = new_tcp_client(database: "test", ssl: true, tls_min_version: Nocturne::TLS_VERSION_13, tls_max_version: Nocturne::TLS_VERSION_13)
    result = client.query "SELECT * FROM performance_schema.session_status WHERE VARIABLE_NAME = 'Ssl_version'"
  ...
  def test_nocturne_connect_ssl_config_cipher_tls13_aesgcm128
    return skip unless tls_1_3_support?

    client = new_tcp_client(database: "test", ssl: true, tls_min_version: Nocturne::TLS_VERSION_13, tls_max_version: Nocturne::TLS_VERSION_13, tls_ciphersuites: "TLS_AES_128_GCM_SHA256")
  ...
  def test_nocturne_connect_ssl_config_cipher_tls13_aesgcm256
    return skip unless tls_1_3_support?

    client = new_tcp_client(database: "test", ssl: true, tls_min_version: Nocturne::TLS_VERSION_13, tls_max_version: Nocturne::TLS_VERSION_13, tls_ciphersuites: "TLS_AES_256_GCM_SHA384")
    result = client.query "SELECT * FROM performance_schema.session_status WHERE VARIABLE_NAME = 'Ssl_cipher'"
  ...
  def test_nocturne_ssl_verify_ca_with_ca
    ...
    client = new_tcp_client(database: "test", ssl_mode: Nocturne::SSL_VERIFY_CA, ssl_ca: "#{ca_cert_path}/ca.pem", tls_max_version: Nocturne::TLS_VERSION_12)
    result = client.query "SELECT * FROM performance_schema.session_status WHERE VARIABLE_NAME = 'Ssl_version'"
    assert_equal [["Ssl_version", "TLSv1.2"]], result.to_a
  ensure
    ensure_closed client
  ...
  def test_nocturne_ssl_verify_identity_with_hostname_match
    ...
    client = new_tcp_client(database: "test", ssl_mode: Nocturne::SSL_VERIFY_IDENTITY, ssl_ca: "#{ca_cert_path}/ca.pem", tls_max_version: Nocturne::TLS_VERSION_12)
    result = client.query "SELECT * FROM performance_schema.session_status WHERE VARIABLE_NAME = 'Ssl_version'"
    assert_equal [["Ssl_version", "TLSv1.2"]], result.to_a
  ensure
    ensure_closed client
