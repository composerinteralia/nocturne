...
class CastTest < NocturneTest
  ...
  def test_disable_casting
    @client.query_flags = Nocturne::QUERY_FLAGS_NONE
  ...
  def test_bit_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (bit_test, single_bit_test) VALUES (0, 0), (1, 1)
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT bit_test, single_bit_test FROM nocturne_test ORDER BY id ASC
    SQL
    ...
    results = @client.query(<<-SQL).to_a
      SELECT single_bit_test FROM nocturne_test ORDER BY id ASC
    SQL
  ...
  def test_tiny_int_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (tiny_int_test, bool_cast_test) VALUES (0, 0), (1, 1)
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT tiny_int_test, bool_cast_test FROM nocturne_test ORDER BY id ASC
    SQL
    ...
    results = @client.query(<<-SQL).to_a
      SELECT bool_cast_test FROM nocturne_test ORDER BY id ASC
    SQL
  ...
  def test_integer_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (small_int_test, medium_int_test, int_test, big_int_test, year_test)
      VALUES (123, 456, 789, 999999999999999999, 1994)
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT small_int_test, medium_int_test, int_test, big_int_test, year_test FROM nocturne_test
    SQL
  ...
  def test_negative_integer_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (small_int_test, medium_int_test, int_test, big_int_test)
      VALUES (-123, -456, -789, -999999999999999999)
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT small_int_test, medium_int_test, int_test, big_int_test FROM nocturne_test
    SQL
  ...
  def test_integer_range_max
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (tiny_int_test, small_int_test, medium_int_test, int_test, big_int_test, unsigned_big_int_test)
  ...
  def test_sum_result_int_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (int_test) VALUES (1), (2), (3)
    SQL
  ...
  def test_float_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (float_test, double_test)
      VALUES (12.34, 56.78)
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT float_test, double_test FROM nocturne_test
    SQL
  ...
  def test_decimal_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (decimal_test)
      VALUES (12.34)
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT decimal_test FROM nocturne_test
    SQL
  ...
  def test_date_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (date_test)
      VALUES ("1980-08-27")
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT date_test FROM nocturne_test
    SQL
  ...
  def test_time_cast_defaults_to_utc
    ...
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (time_test)
      VALUES ("#{time.strftime("%H:%M:%S")}")
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT time_test FROM nocturne_test
    SQL
  ...
  def test_time_cast_local
    ...
    @client.query_flags |= Nocturne::QUERY_FLAGS_LOCAL_TIMEZONE
  ...
  def test_timestamp_cast_defaults_to_utc
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (null_test)
      VALUES (NULL)
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT timestamp_test FROM nocturne_test
    SQL
  ...
  def test_datetime_cast_defaults_to_utc
    ...
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (date_time_test)
      VALUES ("#{time.strftime("%Y-%m-%d %H:%M:%S")}")
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT date_time_test FROM nocturne_test
    SQL
  ...
  def test_datetime_cast_with_precision
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (date_time_with_precision_test)
      VALUES ("2018-06-08 08:38:18.108")
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT date_time_with_precision_test FROM nocturne_test
    SQL
  ...
  def test_time_cast_with_precision
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (time_with_precision_test)
      VALUES ("08:38:18.108")
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT time_with_precision_test FROM nocturne_test
    SQL
  ...
  def test_binary_cast
    @client.query(<<-SQL)
      INSERT INTO nocturne_test (binary_test, varbinary_test)
      VALUES ("hello", "world")
    SQL

    results = @client.query(<<-SQL).to_a
      SELECT binary_test, varbinary_test FROM nocturne_test
    SQL
  ...
  def test_everything_else_casts_to_string
    ...
    text_columns.each do |column|
      ...
      @client.query(<<-SQL)
        INSERT INTO nocturne_test (#{column})
        VALUES ("#{value}")
      SQL

      results = @client.query(<<-SQL).to_a
        SELECT #{column} FROM nocturne_test
      SQL
